 ...
 */
    self = function (pattern, flags) {
        ...
        if (nativ.match.call(flags, duplicateFlags)) { // Don't use api/exec because they would update lastIndex
 ...
 * `RegExp.prototype.api` and can be used reliably cross-browser.
 * @memberOf XRegExp
 * @param {String} str String to search.
 * @param {RegExp} regex Regex to search with.
 * @param {Number} [pos=0] Zero-based index at which to start the search.
 ...
 *
 * // Basic use
 * XRegExp.api('abc', /c/); // -> true
 *
 * // With pos and sticky
 * XRegExp.api('abc', /c/, 0, 'sticky'); // -> false
 */
...
/**
 * Fixes browser bugs in the native `RegExp.prototype.api`. Calling `XRegExp.install('natives')`
...
/**
 * Implicitly calls the regex's `api` method with the first value in the provided arguments array.
...
/**
 * Implicitly calls the regex's `api` method with the provided string.
 * @memberOf XRegExp.prototype
